    ...
    fn from(value: api_models::payments::PaymentListConstraints) -> Self {
        ...
        if let Some(payment_intent_id) = payment_id {
            ...
            Self::List(Box::new(PaymentIntentListParams {
                ...
                connector: connector.map(|c| vec![c]),
                currency: currency.map(|c| vec![c]),
                status: status.map(|s| vec![s]),
                payment_method_type: payment_method_type.map(|p| vec![p]),
                payment_method_subtype: payment_method_subtype.map(|p| vec![p]),
                authentication_type: authentication_type.map(|a| vec![a]),
                merchant_connector_id: merchant_connector_id.map(|m| vec![m]),
                ...
                card_network: card_network.map(|c| vec![c]),
                merchant_order_reference_id,
            }))
        }
    }
